...
def read_regions() -> dict:
    ...
    with open("regions.txt", "r") as f:
        for line in f:
            region, latitude, longitude = line.split(",")
            regions[region] = (float(latitude), float(longitude),)
        return regions
...
def map_locations(locations: pd.Series):
    ...
    # return pd.DataFrame(map_locs, columns=['country_subject', 'position'])
    return map_locs
...
if __name__ == "__main__":
    # r = read_regions()
    # print(r)
    top_players = pd.read_excel("result1.xlsx")
